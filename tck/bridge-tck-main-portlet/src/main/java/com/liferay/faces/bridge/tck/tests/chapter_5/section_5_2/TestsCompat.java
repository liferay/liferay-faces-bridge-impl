 ...
 */
public class TestsCompat extends Object {

	// Test is SingleRequest -- Header/Render/Action
	// Test #5.33 --
	@BridgeTest(test = "headerOrRenderPhaseListenerTest")
	public String headerOrRenderPhaseListenerTest(TestRunnerBean testRunner) {
		...
		if ((lastBeforePhaseId == null) || (lastAfterPhaseId == null)) {
			testRunner.setTestResult(false,
				"Header or Render incorrectly didn't invoke either or both the RESTORE_VIEW before/after listener.");

			return Constants.TEST_FAILED;
		}
		else if ((lastBeforePhaseId == PhaseId.RESTORE_VIEW) && (lastAfterPhaseId == PhaseId.RESTORE_VIEW)) {
			testRunner.setTestResult(true,
				"Header or Render properly invoked the RESTORE_VIEW phase including calling its before/after listeners and didnt' execute any other action phases.");

			return Constants.TEST_SUCCESS;
		}
		else {
			testRunner.setTestResult(false,
				"Header or Render incorrectly executed an action phase/listener post RESTORE_VIEW: lastBeforePhase: " +
