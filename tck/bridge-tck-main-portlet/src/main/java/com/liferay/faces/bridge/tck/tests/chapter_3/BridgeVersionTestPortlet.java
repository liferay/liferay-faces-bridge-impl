...
public class BridgeVersionTestPortlet extends GenericPortlet {
	...
	public void render(RenderRequest request, RenderResponse response) throws PortletException, IOException {
		...
		try {
			...
			if ((name == null) || !name.equals("Portlet 2.0 Bridge for JavaServer Faces 2.2")) {
				resultWriter.setStatus(BridgeTCKResultWriter.FAIL);
				resultWriter.setDetail("Incorrect Specification Title: " + name +
					" should be 'Portlet 2.0 Bridge for JavaServer Faces 2.2'");
			}
			else if ((version == null) || !version.equals("4.0.0")) {
				resultWriter.setStatus(BridgeTCKResultWriter.FAIL);
				resultWriter.setDetail("Incorrect Specification Version: " + version + " should be '4.0.0'");
			}
			else {
