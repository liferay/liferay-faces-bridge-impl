...
<%
	...
	if (renderResponse.getClass().getName().equals("com.liferay.faces.bridge.tck.filter.TCKRenderResponseWrapper")) {
		m.put("com.liferay.faces.bridge.TCK.status", Boolean.TRUE);
		m.put("com.liferay.faces.bridge.TCK.detail", "ExternalContext correctly returns a response object that implements the configured TCKRenderResponseWrapper during a render (dispatch).");
	} else {
		m.put("com.liferay.faces.bridge.TCK.status", Boolean.FALSE);
		m.put("com.liferay.faces.bridge.TCK.detail", "ExternalContext incorrectly returns a response object that doesn't implement the configured TCKRenderResponseWrapper during a render (dispatch). Instead we got: " + renderResponse.getClass().getName());
	}
